"""
Django settings for TurqHeal project.
...
# Database — PostgreSQL only, configured via DATABASE_URL env var
...
if not DATABASE_URL:
    raise RuntimeError(
        ...
        'Example: postgres://user:pass@localhost:5432/turqheal'
    )
...
REST_FRAMEWORK = {
    ...
    'DEFAULT_THROTTLE_RATES': {
        ...
        'anon': '60/minute',
        'user': '300/minute',
        # Scoped — used by views that opt-in via `throttle_scope`
...
# Reliability: re-deliver tasks if a worker dies mid-execution
CELERY_TASK_ACKS_LATE = True
CELERY_TASK_REJECT_ON_WORKER_LOST = True
CELERY_TASK_TRACK_STARTED = True
CELERY_WORKER_PREFETCH_MULTIPLIER = 1  # don't hoard tasks

# Eager mode: run tasks inline. Off by default; tests flip it on via @override_settings.
CELERY_TASK_ALWAYS_EAGER = os.getenv('CELERY_TASK_ALWAYS_EAGER', 'False').lower() in ('true', '1')
CELERY_TASK_EAGER_PROPAGATES = True


# ----------------------------------------------------------------------
# Email — dev defaults to console; SMTP creds come from env in prod
# ----------------------------------------------------------------------
...
DEFAULT_FROM_EMAIL = os.getenv('DEFAULT_FROM_EMAIL', 'TurqHeal <noreply@turqheal.com>')
...
# Activated when DEBUG=False (i.e. on real deployments).
# Local dev keeps these off so we don't need HTTPS during development.
# ----------------------------------------------------------------------
if not DEBUG:
